...
(
	mem_num number(20) primary key,	--회원번호 PK
	...
	ct_code number(20)	REFERENCES CENTER(CT_CODE) --지점코드 FK
);
CREATE SEQUENCE MEM_SEQ;

create table department			--부서정보
(
	dept_code number(20) primary key,	--부서코드PK
...
(
	stf_num number(20) primary key,	--스태프번호 PK
...
( 
	fb_num number(20) primary key,	--글번호 PK
	mem_num number(20) REFERENCES MEMBER(MEM_NUM),--작성자 FK
...
(
	qna_num number(20) primary key,	--글번호 PK
	mem_num number(20) REFERENCES MEMBER(MEM_NUM),--작성자(회원번호) FK
...
(	
	matt_num number(20) primary key, 	--회원출석번호PK
	matt_date date,			--출석일 
	mem_num number(20) REFERENCES MEMBER(MEM_NUM),--회원번호 FK	
	ct_code number(20) REFERENCES CENTER(CT_CODE) --지점코드 FK			--지점코드
);
CREATE SEQUENCE MATT_SEQ;

create table registration			--등록정보
(
	rg_num number(20) primary key,	--등록번호
	mem_num number(20) REFERENCES MEMBER(MEM_NUM),--회원번호 FK
...
(
	tr_num number(20) primary key,	--강사번호 PK
	stf_num number(20) REFERENCES STAFF(STF_NUM), --스태프번호 FK
...
(
	tt_num number(20) primary key,	--시간번호 PK
...
(
	pts_num number(20) primary key,	--PT스케줄번호 PK
	pts_start date,			--PT시작일
	pts_end date,			--PT만료일
	mem_num number(20) REFERENCES MEMBER(MEM_NUM), --회원번호FK
	tt_num number(20) REFERENCES TIMETABLE(TT_NUM), --시간번호FK
	tr_num number(20) REFERENCES TRAINER(TR_NUM), --강사번호FK
	ct_code number(20) REFERENCES CENTER(CT_CODE)  --지점코드FK
);
CREATE SEQUENCE PTS_SEQ;

create table pt_log				--운동일지
(
	ptl_num number(20) primary key,	--일지번호 PK
	ptl_content varchar2(4000),		--일지내용
	pts_num number(20) REFERENCES PT_SCHEDULE(PTS_NUM) --PT스케줄번호 FK
);
CREATE SEQUENCE PTL_SEQ;

create table medicalcheck			--의료정보
(
	md_num number(20) primary key,	--md번호 PK
	mem_num number(20) REFERENCES MEMBER(MEM_NUM), --회원번호 FK
...
(	
	sal_num number(20) primary key,	--매출번호 PK
...
(
	Satt_num number(20) primary key, 	--직원출석번호PK
	satt_date date,			--출석일 
	stf_num number(20) REFERENCES STAFF(STF_NUM), --직원번호 FK
...
(
	pay_num number(20) primary key,	--월급번호 PK
	pay_tot number,		--월급합계
	pay_date date,			--지급날짜
	satt_num number(20) REFERENCES STF_ATT(SATT_NUM) --출석일 FK
);
CREATE SEQUENCE PAY_SEQ;

create table notice				--공지사항	
(
	nt_num number(20) primary key,	--글번호 PK
	stf_num number(20) REFERENCES STAFF(STF_NUM), --스태프 번호 FK
...
(
	gx_num number(20) primary key,	--GX스케줄번호 PK
	sb_code varchar2(20) REFERENCES SUBJECT(SB_CODE), --과목코드 FK
	gx_start date,			--강의시작일
	gx_end date,			--강의만료일
	tt_num number(20) REFERENCES TIMETABLE(TT_NUM), --시간번호 FK
	tr_num number(20) REFERENCES TRAINER(TR_NUM), --강사번호 FK
	ct_code number(20) REFERENCES CENTER(CT_CODE) --지점코드 FK
);
...
(
	ev_num number(20) primary key,	--이벤트번호 PK
